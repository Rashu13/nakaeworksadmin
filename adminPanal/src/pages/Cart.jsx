        ...
        <div className="min-h-screen bg-gray-50 dark:bg-slate-950 pt-32 pb-20 px-4 sm:px-6 transition-colors duration-300">
            <div className="max-w-6xl mx-auto">
                <div className="flex items-center gap-4 mb-10">
                    <button
                        onClick={() => navigate(-1)}
                        className="p-3 bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl text-gray-600 dark:text-gray-400 hover:text-black dark:hover:text-white transition-all shadow-sm"
                    >
                        <ChevronLeft size={20} />
                    </button>
                    <div>
                        <h1 className="text-3xl font-black text-gray-900 dark:text-white uppercase tracking-tight">Shopping Cart</h1>
                        <p className="text-sm text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider">{totalItems} {totalItems === 1 ? 'item' : 'items'} selected</p>
                    </div>
                </div>
                ...
                <div className="grid lg:grid-cols-12 gap-10">
                            ...
                            <div key={item.id} className="group bg-white dark:bg-slate-900 p-5 rounded-3xl border border-gray-100 dark:border-white/5 shadow-xl shadow-black/5 flex flex-col sm:flex-row items-center gap-6 transition-all hover:border-orange-500/20">
                                    ...
                                    <div className="flex items-center justify-center sm:justify-start gap-4">
                                        <div className="flex items-center gap-2 bg-gray-50 dark:bg-white/5 p-1.5 rounded-xl border border-gray-200/50 dark:border-white/5">
                                            <button
                                                ...
                                                className="w-8 h-8 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white transition-colors"
                                            >
                                                <Minus size={14} />
                                            </button>
                                ...
                                <div className="text-right flex flex-col items-center sm:items-end justify-between self-stretch py-1">
                                    ...
                                    <div className="mt-4 sm:mt-0">
                                        <p className="text-xl font-black text-gray-900 dark:text-white tracking-tighter">₹{Math.round((item.price - (item.discount || 0)) * item.quantity)}</p>
                                        <p className="text-[10px] text-gray-400 dark:text-gray-500 font-bold uppercase tracking-widest">₹{Math.round(item.price - (item.discount || 0))} each</p>
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="lg:col-span-4">
                        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-gray-100 dark:border-white/5 shadow-2xl shadow-black/5 sticky top-32 transition-all">
                            <h2 className="text-xl font-black text-gray-900 dark:text-white mb-8 uppercase tracking-tight">Order Summary</h2>
                            <div className="space-y-4 mb-8">
                                <div className="flex justify-between text-sm font-bold uppercase tracking-wider">
                                    <span className="text-gray-500 dark:text-gray-400">Subtotal</span>
                                    <span className="text-gray-900 dark:text-white">₹{Math.round(cartTotal)}</span>
                                </div>
                                <div className="flex justify-between text-sm font-bold uppercase tracking-wider">
                                ...
                                <div className="pt-4 border-t border-gray-100 dark:border-white/5 flex justify-between">
                                    ...
                                    <span className="text-2xl font-black text-gray-900 dark:text-white tracking-tighter">₹{Math.round(cartTotal)}</span>
                                </div>
                            </div>
